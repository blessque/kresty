...
export class RentScreen extends ContentScreen {
  ...
  protected build() {
    ...
    // the heading says so.
    body.innerHTML =
      `<div class="col-aside sticky-head gp-text"><h2 class="page-h2">О комплексе</h2></div>` +
      `<div class="col-main page-flow gp-text">` +
      `<p class="page-prose">${escapeHtml(bindShortWords(BODY))}</p>` +
      `</div>`;
    this.shell.add(body);

    // the shared form, with «Аренда»'s own heading. `data-sticky-head` is what
    // makes its `.sec-col` pin by MEASUREMENT rather than at the flat `14vh`
    // fallback — without it this page had one heading centred at a guess and two
    // not sticky at all, which is the bug «Контакты» reported in round 27.
  ...
  private buildSpaces(): HTMLElement {
    ...
    section.dataset.stickyHead = '';

    const aside = document.createElement('div');
    aside.className = 'col-aside sticky-head gp-text';
    ...
    SPACES.forEach((s, i) => {
      ...
      // NOT `.gp-text` — this wrapper holds the photograph as well as the prose,
      // and the inset would be inherited by the image (grid.css states the rule
      // and the round-25 bug it came from). The text children carry it instead.
   ...
   * heights, so a switch moves every colour stop below it AND the handoff's
   * `offsetTop` — leave it out and the seam fires at the wrong scroll position.
   * `NewsScreen.applyFilter` ends the same way for the same reason. It also
   * re-runs `heads.measure()` through `onMeasure`, re-pinning the column.
   */
